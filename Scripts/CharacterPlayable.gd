...
func _enter_tree():
	...
	if multiplayer.get_unique_id() == player_id:
		character_int = PlayerProfile.get_character()
		hair_int = PlayerProfile.get_character()
		tops_int = PlayerProfile.get_character()
		pants_int = PlayerProfile.get_character()
...
func _ready():
	...
	if character_int == 1:
		$Armature/Skeleton3D/Character_001.mesh = load("res://Characters/Tiana/TianaBase.tres")
...
func loadup_outfit():
	if character_int == 1:
		if PlayerProfile.get_character() == 1:
...
func _physics_process(delta):
	...
	if is_sliding:
		...
		if result:
			...
			var gravityVec = Vector3.DOWN

			var slope_direction = (gravityVec - ground_normal * gravityVec.dot(ground_normal)).normalized()
...
func find_obstacle_top(from_pos: Vector3, dirr: Vector3, min_y: float, max_y: float, tolerance: float = 0.05) -> float:
	var space_state = get_world_3d().direct_space_state
	
	while abs(max_y - min_y) > tolerance:
		var mid_y = (min_y + max_y) / 2.0
		var ray_origin = from_pos + Vector3(0, mid_y, 0)
		var ray_end = ray_origin + dirr * 2.5  # or desired ray length
...
func spawn_debug_marker(pos: Vector3, text := ""):
